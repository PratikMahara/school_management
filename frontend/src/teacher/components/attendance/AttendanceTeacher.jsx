...
const AttendanceTeacher = () => {
  ...
  const handleClassChange = (event) => {
    ...
    setSelectedClass({id:input.split(",")[0], class_section:input.split(",")[1]});
  ...
  return (
    <Container>
     ...
     {attendeeClass.length>0 && <FormControl fullWidth sx={{ mb: 3 }}>
        ...
        <Select value={selectedClass?`${selectedClass.id},${selectedClass.class_section}`:""} onChange={handleClassChange}>
          <MenuItem value={''}>Select Class</MenuItem>
          {attendeeClass && attendeeClass.map((student_class,i)=>(
             <MenuItem key={i} value={`${student_class.classId},${student_class.class_section}`}>{student_class.class_section}</MenuItem>
          ))}
      ...
      {attendeeClass.length>0 && selectedClass && attendanceTaken && (
        <Alert severity="info" sx={{ mb: 3 }}>
          Attendance has already been taken for today for Class {selectedClass.class_section}
        </Alert>
      ) }
        {attendeeClass.length>0 && selectedClass && !attendanceTaken && students.length<1 && (
            <Alert severity="info" sx={{ mb: 3 }}>
            There is no students in { selectedClass.class_section} class now.
          </Alert>
        )}
